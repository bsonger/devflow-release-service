...
type deployTargetOwnerReader interface {
	GetApplication(ctx context.Context, id string) (*downstream.AppApplication, error)
	GetProject(ctx context.Context, id string) (*downstream.AppProject, error)
	GetEnvironment(ctx context.Context, id string) (*downstream.AppEnvironment, error)
	GetCluster(ctx context.Context, id string) (*downstream.AppCluster, error)
}
...
func newDeployTargetResolver(cfg RuntimeConfig) *deployTargetResolver {
	return &deployTargetResolver{
		...
		ownerReader:   downstream.NewAppManifestClient(strings.TrimSpace(cfg.Downstream.AppServiceBaseURL)),
	}
}
...
func (r *deployTargetResolver) Resolve(ctx context.Context, applicationID, environmentID string) (*deployTarget, error) {
	...
	namespace, err := deriveNamespace(project.Name, environment.Name)
	if err != nil {
