...
type stubIntentService struct {
	getFn  func(context.Context, uuid.UUID) (*model.Intent, error)
	listFn func(context.Context, service.IntentListFilter) ([]*model.Intent, error)
}

func (s stubIntentService) Get(ctx context.Context, id uuid.UUID) (*model.Intent, error) {
	return s.getFn(ctx, id)
}

func (s stubIntentService) List(ctx context.Context, filter service.IntentListFilter) ([]*model.Intent, error) {
...
func TestListIntentReturnsEnvelope(t *testing.T) {
	...
	handler := &IntentHandler{
		svc: stubIntentService{
			listFn: func(_ context.Context, filter service.IntentListFilter) ([]*model.Intent, error) {
				if filter.Kind != string(model.IntentKindBuild) {
					t.Fatalf("unexpected filter: %#v", filter)
				}
				return []*model.Intent{{Kind: model.IntentKindBuild, Status: model.IntentPending, ResourceType: "manifest", ResourceID: uuid.New()}}, nil
			},
		},
	}
