...
| `steps` | `[]ImageStep` | system-managed | system | 构建步骤状态 |
| `status` | `ImageStatus` | system-managed | system | 构建状态 |
...
Historical image rows can be purged with `devflow-control/scripts/cleanup-image-manifests.sh`.
